...
class BaseSKLogicEstimator:
    ...
    def _handle_non_dataframe_targets(y):
        """Coerce various target shapes/types into a single-column float DataFrame."""
        # If already a DataFrame, ensure float32 dtype and return
        if isinstance(y, pd.DataFrame):
            if y.shape[1] == 1:
                col = y.columns[0]
                y[col] = y[col].astype(np.float32)
                return y
            # Multi-target case: cast all to float32
            return y.apply(lambda col: col.astype(np.float32))

        # If Series -> single-column DataFrame
        if isinstance(y, pd.Series):
            return pd.DataFrame({y.name or 'target': y.astype(np.float32)})
        ...
        if isinstance(y, _np.ndarray):
            if y.ndim == 1:
                return pd.DataFrame({'target': y.astype(_np.float32)})
            elif y.ndim == 2 and y.shape[1] == 1:
                return pd.DataFrame({'target': y.reshape(-1).astype(_np.float32)})
            else:
                # Multi-target numpy array
                return pd.DataFrame(y.astype(_np.float32), columns=[f'Class {i}' for i in range(y.shape[1])])

        # Fallback: try to construct a DataFrame directly
        return pd.DataFrame({'target': y}).astype(_np.float32)

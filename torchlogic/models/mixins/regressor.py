...
class ReasoningNetworkRegressorMixin(BaseReasoningNetworkMixin):
    ...
    def explain(
        ...
        if output_channel is None:
            output_indices = range(self.output_size)
        else:
            if output_channel < 0 or output_channel >= self.output_size:
                raise ValueError(f"output_channel must be in [0, {self.output_size - 1}]")
            output_indices = [output_channel]
        ...
        for output in output_indices:
            ...
            if inverse_transform_target is not None:
                display_threshold = inverse_transform_target(required_output_thresholds.reshape(1, -1))
            ...
            target_label = target_names[output] if len(target_names) > output else f"output_{output}"
            explanation += [f"{explanation_prefix} predicted {target_label} "
                            f"of {round(float(display_threshold), rounding_precision)} "
                            f"because: \n\n{explanation_str}"]
    ...
    def explain_samples(
        ...
        for i in range(x.size(0)):
            ...
            explanation = []
            parent_weights = torch.tensor(1.)
            required_output_threshold = predictions[0, output_channel].detach().cpu().squeeze()

            if boosted_predictions is not None:
                prediction_value = boosted_predictions.squeeze()[output_channel]
            else:
